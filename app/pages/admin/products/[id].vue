...
const { data: product, pending } = useAsyncData(`admin-product-edit-${id}`, async () => {
...
useSeoMeta({
  title: computed(() =>
    product.value ? `Edit "${product.value.translations?.en?.title ?? 'Product'}" — Velora Admin` : 'Edit Product — Velora Admin'
  ),
})
          ...
          <div class="flex items-start gap-4">
            ...
            <div>
              ...
              <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                ...
                <span class="font-medium text-zinc-700 dark:text-zinc-300">{{ product?.translations?.en?.title }}</span>?
